...
struct Buffer
{
	...
	virtual BOOL AllocateBuffer(ULONG nBufferSize)
	{
		if ( nBufferSize == 0)
			return FALSE;
	...
	virtual BOOL ExtendBuffer(ULONG nBufferSize)
	{
	...
	virtual BOOL FillData(PBYTE pData, ULONG nDataSize)
	{
	...
	virtual BOOL AppendData(PBYTE pData, ULONG nDataSize)
	{
		if (pData == NULL || nDataSize == 0) 
			return FALSE;
